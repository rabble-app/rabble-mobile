...
abstract class RabbleHttpError {
  final int code;
  ...
  factory RabbleHttpError.fromDioError(DioError e) {
...
class ValidationError {
  ...
  // That way we can return it from Rabble HttpError.fromDioError
...
// TODO add defaults for non-nullable properties and ? to nullable
class ConflictError extends RabbleHttpError {
  final dynamic error;
...
// TODO add defaults for non-nullable properties and ? to nullable
class BadRequestError extends RabbleHttpError {
  final dynamic error;
...
extension ErrorHandling on DioError {
  ...
  bool isHttpError() {
    ...
    return this.type == DioErrorType.badResponse;
  }

  bool isBadRequestError() =>
      response?.statusCode == RabbleHttpError.BAD_REQUEST;

  bool isConflictError() => response?.statusCode == RabbleHttpError.CONFLICT;

  bool isValidationError() =>
      response?.statusCode == RabbleHttpError.UNPROCESSABLE;

  bool isNotFoundError() => response?.statusCode == RabbleHttpError.NOT_FOUND;

  bool isUnauthenticatedError() =>
      response?.statusCode == RabbleHttpError.UNAUTHENTICATED;

  bool isUnauthorizedError() =>
      response?.statusCode == RabbleHttpError.UNAUTHORIZED;

  bool isServerError() => response?.statusCode == RabbleHttpError.SERVER;
  ...
  bool isConnectionError() {
    ...
    if (type == DioErrorType.unknown && this.error is SocketException) {
      return true;
    }
    return [
